...
- A new photo goes into `public/` as WebP at the lowest quality that still scores SSIMULACRA2 85 or more against the original at full width (libjxl's tool; 85 means no visible difference when flipping between the two at 1:1). Full width because a 2x screen asks next/image for more pixels than the file has, so it gets the file's full width, re-encoded at q75.
