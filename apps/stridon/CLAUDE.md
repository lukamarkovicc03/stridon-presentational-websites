...
- **This branch is PR #14 against `upstream/main`, and that PR's description is the handover document** - the architecture decisions, the blast radius on dck and sg-tools, the SEO carry-over table, the issues that are not ours, and what was deliberately declined. It was opened 2026-09-07 and its description was rewritten on 2026-09-22 when the branch had moved from 10 commits to 42; do not open a second PR. Two things wait on the repo owner rather than on this app: authorizing the fork's Vercel preview, and a production build of dck and sg-tools on Next 16.3.5, which is the one unverified claim in the whole PR.
...
- **Done (handover audit, 2026-09-22): a full production-readiness sweep before the PR, which found one real bug and four smaller things.** The bug has its own section below: an unvalidated locale made ten legacy asset URLs answer **500 instead of 404**, and only in a production build, so no amount of local clicking found it. The other four: `/sitemap.xml` was building as a function because of a `new Date()`, and all 68 `lastmod` values were the same "now", so `lastModified` is gone and the route is static; the language switch emitted `hreflang="sr"` while every document-level alternate uses `sr-Latn`; two English brand descriptions ran to 167 characters, because the length sweep that produced "nothing over the limit" had only covered Serbian; and `NEXT_PUBLIC_CARTO_BASEMAPS_KEY` was missing from this app's `.env.example`, since it landed in dck and sg-tools with `6b95543` before this app existed, while `/servis` renders a map through the same shared `addBrandTileLayer`. Two cross-brand changes came out of it as well, in `packages/shared`: the navbar's tighter mobile spacing now rides on the `languageSwitch` slot instead of applying to all three sites, so dck and sg-tools render byte-identically to `upstream/main` again, and the mobile menu trigger got the `aria-label` it never had on any of the three. **Verified clean in the same pass:** no `localhost`, LAN IP or `:3000` anywhere in shipped code or in any of the 86 prerendered HTML files; no hardcoded secret, and `.env.local` untracked; every one of the 68 sitemap URLs returns 200 and matches `routing.ts`; metadata coverage 100% across all 68 real pages with every absolute URL on the production origin; exactly one `h1` per page with no skipped level; and `tsc --noEmit` clean on **all three** apps with `pnpm preflight` 7/7.
...
- Pending, raised and not acted on: no CSP header anywhere in the monorepo (a report-only draft sized for this app's actual sources is in the chat history), unescaped HTML in the shared `sendContactEmail`, `API_URL` in the Vercel env before deploy (the build **hard-fails** without it, it is not a degraded page), `NEXT_PUBLIC_CARTO_BASEMAPS_KEY` in the same place or `/servis` shows a watermarked map, and a PACMS webhook to `revalidateTag` so a CMS edit is not up to 24h behind.
